namespace ININ.Testing.Automation.Test.Client.TestCases.TS2493
{
    ...
    /// <summary>
    ///     TC29595 - Dial Business Phone 2 from Properties view
    /// </summary>
    public class TC29595 : TS2493TestCase
    {
            ...
            using (Trace.TestCase.scope())
            {
                using (Rm = ResourceManagerRuntime.AllocateResources(2, 2))
                {
                    try
                    {
                        ...
                        #region STEP 1: Dial the Business Phone 2 from the 'Business' view.
                        using (Trace.TestCase.scope("Step 1: Dial the Business Phone 2 from the 'Business' view."))
                        {
                            //Step 1 Verify: The correct Business Phone 2 number configured is dialed and appears in the My Interactions queue.
                            ...
                            TraceTrue(
                                propertiesPage.Business.BusinessPhone2Number.DialButton.WaitUntil(WaitUntilType.CanInteract),
                                "The Business Phone 2 dial button was not enabled/visible");

                            propertiesPage.Business.BusinessPhone2Number.DialButton.Click();
        ...
        [Trait("Priority", "P2")]
        [Trait("BFT", "false")]
        public void Test29595_DialBusinessPhone2FromPropertiesView()
        {
            try
            {
                Run();
